/**
 * MESA-Net Firmware — Seeed XIAO ESP32-S3
 *
 * Multimodal Edge Sensing and Adaptation Network
 * Real-time simultaneous classification of:
 *   - Physical Activity  (4 classes: Sedentary / Walking / Cycling / HighIntensity)
 *   - Psychological Stress (2 classes: Baseline / Stress)
 *   - Cardiac Arrhythmia  (2 classes: Normal / Abnormal)
 *
 * Sensors:
 *   - AD8232   ECG analog front-end       (ADC,  GPIO1 / D0)
 *   - MPU6050  6-axis IMU                 (I2C,  SDA=GPIO5/D4, SCL=GPIO6/D5)
 *   - MAX30102 PPG / Pulse Oximeter       (I2C,  shared bus)
 *   - GSR/EDA  Skin Conductance (optional)(ADC,  GPIO2 / D1)
 *
 * Platform: Seeed XIAO ESP32-S3 (ESP32-S3R8, 240 MHz, 8 MB PSRAM, 8 MB Flash)
 * Serial:   USB CDC at 115200 baud (/dev/cu.usbmodem* on macOS)
 * Window:   10 seconds × 1000 samples @ 100 Hz → inference → adaptation
 */
...
#include "../include/pins_xiao_esp32s3.h"
...
static void initializeSensors() {
    ...
    if (!i2c_ok) {
        // XIAO ESP32-S3 fallback: try D9/D8 (GPIO8/GPIO7)
        CONSOLE.printf("I2C init FAILED on SDA=%d SCL=%d; trying fallback SDA=8 SCL=7\n",
                       used_sda, used_scl);
        used_sda = 8;
        used_scl = 7;
    ...
    // the wrong pin or the OUT line is shorted / not grounded.
    {
        ...
        if (mn < 20 || mx > 4075 || (mx - mn) < 5) {
            ...
            CONSOLE.println("  - AD8232 OUT must go to D0 (GPIO1) on XIAO ESP32-S3.");
            CONSOLE.println("  - Common GND between AD8232 and XIAO.");
            CONSOLE.println("  - ADC pins on XIAO: D0(GPIO1), D1(GPIO2), D2(GPIO3) only.");
            CONSOLE.println("  - Do NOT use D4/D5 (GPIO5/6 = I2C SDA/SCL).");
            CONSOLE.println("  - LO+ → D3(GPIO4), LO- → D2(GPIO3). Not 3.3V/GND.");
        }
    }
}
...
void setup() {
    ...
    for (int i = 0; i < 2; i++) {
        CONSOLE.printf("BOOT: MESA-Net (XIAO ESP32-S3) starting... (%d/2)\n", i + 1);
        delay(200);
    }

    CONSOLE.println();
    CONSOLE.println("╔═══════════════════════════════════════════╗");
    CONSOLE.println("║  MESA-Net v1.0 — Seeed XIAO ESP32-S3     ║");
    CONSOLE.println("║  Activity | Stress | Arrhythmia           ║");
    CONSOLE.println("╚═══════════════════════════════════════════╝");
    CONSOLE.println();
