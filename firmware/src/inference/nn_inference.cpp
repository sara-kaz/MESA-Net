...
#include "../training/gradient_engine.h"
...
NNResult NNInference::predict(const float* input) {
    ...
    // When adaptive training is active and weights have been modified,
    // use the gradient engine's adapted weights instead of flash ROM.

#if ENABLE_ADAPTIVE_TRAINING
    if (gradientEngine.isReady() && gradientEngine.hasAdaptedWeights()) {
        // Use adapted weights via gradient engine (includes FC1 from flash + adapted LN/FC2/FC3)
        // Activity
        gradientEngine.inferWithAdapted(0, _projected, result.activity_probs);
        // inferWithAdapted already applies softmax
        ...
        // Stress
        gradientEngine.inferWithAdapted(1, _projected, result.stress_probs);
        // Save raw logits before calibration (after softmax, approximate from probs)
        memcpy(result.stress_logits, result.stress_probs, sizeof(result.stress_logits));
        // inferWithAdapted already applies softmax, but re-apply calibration if needed
        if (_calibrated) {
            // Undo softmax → apply bias → re-softmax (approximate: just shift probs)
            // More precisely: log the probs to get logits, subtract bias, re-softmax
            float logits[NNConfig::STRESS_CLASSES];
            for (int i = 0; i < NNConfig::STRESS_CLASSES; i++)
                logits[i] = logf(result.stress_probs[i] + 1e-8f) - _stress_bias[i];
            softmax(logits, NNConfig::STRESS_CLASSES);
            memcpy(result.stress_probs, logits, sizeof(result.stress_probs));
        }
        ...
        // Arrhythmia
        gradientEngine.inferWithAdapted(2, _projected, result.arrhythmia_probs);
        memcpy(result.arrhythmia_logits, result.arrhythmia_probs, sizeof(result.arrhythmia_logits));
        if (_calibrated) {
            float logits[NNConfig::ARRHYTHMIA_CLASSES];
            for (int i = 0; i < NNConfig::ARRHYTHMIA_CLASSES; i++)
                logits[i] = logf(result.arrhythmia_probs[i] + 1e-8f) - _arr_bias[i];
            softmax(logits, NNConfig::ARRHYTHMIA_CLASSES);
            memcpy(result.arrhythmia_probs, logits, sizeof(result.arrhythmia_probs));
        }
...
#endif
    // Default path: use flash ROM weights (original or when adaptive training disabled)
